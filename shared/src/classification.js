...
export const APP_CLASSIFICATION_RULES = [
    ...
    // Browser-based development work (check window titles for work indicators)
    // This must come BEFORE generic browser rule
    {
        patterns: [
            ...
            'mohltbot', 'selfhosted', 'r/selfhosted',
        ],
        category: 'core_work'
    },
    // System/Browser (neutral)
    {
        patterns: [
            ...
            'new tab', 'google search', 'chrome', 'safari', 'firefox', 'edge', 'brave', 'opera',
        ],
        category: 'other'
    }
];
...
export function detectGamingAttempts(activities) {
    ...
    const hasYouTube = activities.some(a => a.appName.toLowerCase().includes('youtube') && a.duration > 900 // 15 min
    );
